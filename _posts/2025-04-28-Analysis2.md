...
증명: Suppose $$B \in \mathbb{R}^d$$ is not compact<br>
...
### Corollary 2.5.8. 셀수있는 닫힌집합 $$S \in \mathcal{R}^d$$는 고립점이 있다. 

## 2.6. Connected Set

### Definition 2.6.1. Connected Set

다음을 만족하는 $S \in mathbb{R}$ 의 열린집합 $U,V$가 존재하지 않는다면 $S$는 연결집합<sub>connected set</sub>
...
닫힌집합 $F \in \mathbb{R}^d$는 가산집합과 완전집합의 합집합으로 표현할 수 있다.
